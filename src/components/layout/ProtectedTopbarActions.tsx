...
export default function ProtectedTopbarActions({ locale, userName }: ProtectedTopbarActionsProps) {
  ...
  return (
    <div ref={wrapperRef} className="relative flex items-center gap-2">
      ...
      <div className="relative">
        <button type="button" aria-label="Profile" className={iconButton} onClick={() => toggleMenu('user')}>
        ...
        {activeMenu === 'user' ? (
          <div className="absolute right-0 mt-2 w-56 rounded-md border border-slate-200 bg-white py-2 shadow-lg">
            <div className="flex items-center gap-3 px-4 py-3 text-slate-600 border-b border-slate-100">
              <svg viewBox="0 0 24 24" className="h-6 w-6" fill="none" stroke="currentColor" strokeWidth="1.8">
                <circle cx="12" cy="8" r="3" />
                <path d="M5 20c1.5-3 4-4.5 7-4.5s5.5 1.5 7 4.5" />
              </svg>
              <span className="text-base font-medium">{userName ?? 'Admin User'}</span>
            </div>
